             ...
             if(langcode === "es") {
                 if(args[0] === 'all') {
                    ...
                    roles.forEach(role => {
                     ...
                     message.channel.send('El envío de mensajes en este canal ha sido restablecido para todos los roles (inferiores al mío)')
                 } else if(args[0]) {
                 let role = message.guild.roles.cache.find(r => r.name === args.join(' ')) || message.guild.roles.cache.get(args[0])
                 message.channel.createOverwrite(role, {
                    SEND_MESSAGES: null
                           });
                 message.channel.send(`El envío de mensajes en este canal ha sido restablecido para \`\`${role.name}\`\``)
                 } else if(!args[0]) {
                    let role = message.guild.roles.cache.find(r => r.name === '@everyone')
                    message.channel.createOverwrite(role, {
                       SEND_MESSAGES: null
                              });
                    message.channel.send(`El envío de mensajes en este canal ha sido restablecido para \`\`everyone\`\``)
                 }
                ...
                if(args[0] === 'all') {
                   ...
                   roles.forEach(role => {
                    ...
                    message.channel.send('SEND_MESSAGES permission has been re-established for all the roles (lower than mine)')
                } else if(args[0]) {
                let role = message.guild.roles.cache.find(r => r.name === args.join(' ')) || message.guild.roles.cache.get(args[0])
                message.channel.createOverwrite(role, {
                   SEND_MESSAGES: null
                          });
                message.channel.send(`SEND_MESSAGES permission has been re-established for \`\`${role.name}\`\``)
                } else if(!args[0]) {
                   let role = message.guild.roles.cache.find(r => r.name === '@everyone')
                   message.channel.createOverwrite(role, {
                      SEND_MESSAGES: null
                             });
                   message.channel.send(`SEND_MESSAGES permission has been re-established for \`\`everyone\`\``)
                }
            }
        }
    });
