            ...
            if(!rol) return message.channel.send('Error, name or id') 
               ...
               NuevoModelo.save().then(data => {
                 message.channel.send(`Ok.`)
                   ...
                   datos.modrole = rol.id
                   datos.save().then(nuevo_dato => { 
                     if(langcode === "es") {
                       message.channel.send(`Ok, rol ${message.guild.roles.cache.get(nuevo_dato.modrole).name} establecido como el rol de moderador.`)
                     } else if(langcode === "en") {
                      message.channel.send(`Ok, role ${message.guild.roles.cache.get(nuevo_dato.modrole).name} set as moderator role.`)
                     }
                   });
                  }
                });
