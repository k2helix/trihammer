...
module.exports = async (client, oldChannel, newChannel) => {
  ...
  let langcode = serverConfig.lang
      ...
      let tipo = ''
      const entry1 = await oldChannel.guild.fetchAuditLogs().then(audit =>
        audit.entries.first())
      ...
      oldChannel.guild.roles.cache.forEach(role => {
        ...
        if (langcode === "es") {
          if (nuevoperm[0] && permremoved[0]) {
            return logs_channel.send(`${user} ha cambiado los permisos del rol \`\`${role.name}\`\` en el canal <#${oldChannel.id}>, se le ha añadido: \`\`${nuevoperm.join(', ')}\`\`. Se le ha removido: \`\`${permremoved.join(', ')}\`\``)
          } else if (permremoved[0]) {
            return logs_channel.send(`${user} ha cambiado los permisos del rol \`\`${role.name}\`\` en el canal <#${oldChannel.id}>, se le ha removido: \`\`${permremoved.join(', ')}\`\``)
          } else if (nuevoperm[0]) {
          ...
          if (nuevoperm[0] && permremoved[0]) {
            return logs_channel.send(`${user} has updated \`\`${role.name}\`\` role permissions in the channel <#${oldChannel.id}>, added: \`\`${nuevoperm.join(', ')}\`\`. Removed: \`\`${permremoved.join(', ')}\`\` `)
          } else if (permremoved[0]) {
