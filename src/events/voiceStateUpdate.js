...
module.exports = async (client, oldState, newState) => {
	...
	if (oldChannel !== newChannel)
		if (langcode === 'es') {
			...
			else if (newChannel === null) logs_channel.send(`:red_circle: ${newState.member.user.tag} (${newState.id}) ha abandonado el canal de voz ${oldChannel.name}`);
			...
			else if (newChannel === null) logs_channel.send(`:red_circle: ${newState.member.user.tag} (${newState.id}) has left voice channel ${oldChannel.name}`);
			else if (oldChannel && newChannel)
				logs_channel.send(
