...
module.exports = async (client, message) => {
	...
	if (message.channel.type == 'dm') {
		...
		canal.send(`Mensaje de ${message.author.tag} recibido: \n \`${message.content}\``);
		return;
	}
	...
	require('../utils/methods/experience').giveXp(message, xp);

	require('../utils/methods/functions').prinsjoto(message);
	...
	if (command.cooldown) {
		...
		if (timestamps.has(message.author.id)) {
			...
			if (now < expirationTime) {
				...
				return message.reply(util.cooldown(timeLeft.toFixed(1), command.name));
			}
		}
	...
	try {
		...
		setTimeout(() => {
			let logschannel = serverConfig.actionslogs;
			...
			return cmdObj.send(config.command_used.replaceAll(cmdObj));
		}, 1000);
	} catch (error) {
		console.error(error);
	}
