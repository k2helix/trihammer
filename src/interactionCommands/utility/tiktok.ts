...
export default new MessageCommand({
	...
	async execute(client, interaction, guildConf) {
		...
		try {
			...
			do {
				let r = await request.get(`${instances[currentInstance]}/@${(interaction as ChatInputCommandInteraction).options.getString('user')}`);
			...
			if (!currentPost && currentInstance === instances.length) {
				let { text } = await request.get(`https://www.tiktok.com/node/share/user/@${(interaction as ChatInputCommandInteraction).options.getString('user')}?aid=1988`, {
				...
				if (parsedData.statusCode !== 0)
					return interaction.reply({ embeds: [client.redEmbed(`[${parsedData.statusCode}] An error ocurred while getting user posts.\n${parsedData.errMsg}`)] });
			...
			if (!currentPost) return interaction.reply({ embeds: [client.redEmbed(music.not_found)] });
