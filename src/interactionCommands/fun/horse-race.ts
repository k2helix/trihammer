...
const { horses } = require('../../lib/utils/objects');
module.exports = {
	name: 'horse-race',
	...
	async execute(client, interaction, guildConf) {
		...
		interaction.reply(`<@${interaction.user.id}>, ` + `${util.horse_race.choose_horse}\n${chosenHorses.map((horse, i) => `**${i + 1}.** ${horse.name}`).join('\n')}`);
		const filter = (res) => {
			if (res.author.id !== interaction.user.id) return false;
		...
		if (!msgs.size) return interaction.followUp({ content: `<@${interaction.user.id}>, ` + util.horse_race.no_bets, ephemeral: true });
		...
		return interaction.followUp({
			content: `<@${interaction.user.id}>, ` + (win ? util.horse_race.win : util.horse_race.lose),
			files: [leaderboard]
		});
	}
};
