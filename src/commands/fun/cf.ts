...
export default new MessageCommand({
	...
	aliases: ['cockfight', 'cock-fight', 'chickenfight', 'chicken-fight'],
	category: 'fun',
	async execute(client, message, _args, guildConf) {
		...
		let msg = await message.channel.send({ embeds: [client.lightBlueEmbed(util.cf.description).setTitle(util.cf.cf)], components: [row] });
		...
		collector.on('collect', async (reaction) => {
			...
			const filter = (interaction: ModalSubmitInteraction) => interaction.customId === 'cf-join' && interaction.user.id === reaction.user.id;
			reaction
				.awaitModalSubmit({ filter, time: closesAt - Date.now() })
				.then((interaction) => {
					cock.name = interaction.fields.getTextInputValue('cf-name');
					interaction.reply({ embeds: [client.blueEmbed(util.cf.joined)], ephemeral: true });
				})
				.catch(() => false);
		...
		collector.on('end', async (collected) => {
			if (collected.size < 2 || participants.length < 2) {
				msg.delete();
				return message.channel.send({ embeds: [client.redEmbed(util.cf.no_participants)] });
			}
			...
			await sleep(1000);
			message.channel.send({ embeds: [client.lightBlueEmbed(util.cf.starting)] });
			await sleep(3000);

			let roundInfo = generateMovement(participants, util);
			message.channel.send({ embeds: [client.blackEmbed(util.cf.first_round + ' ' + roundInfo.msg).setColor(roundInfo.color)] });

			do {
				await sleep(5000);
				roundInfo = generateMovement(participants, util);
				message.channel.send({ embeds: [client.blackEmbed(roundInfo.msg).setColor(roundInfo.color)] });

				await sleep(1000);
				roundInfo.losers.forEach((c) => {
					message.channel.send({ embeds: [client.redEmbed(util.cf.chicken_lost(c))] });
				});
			} while (!roundInfo.end);
			...
			if (!winner) message.channel.send({ embeds: [client.lightBlueEmbed(util.cf.draw)] });
			else message.channel.send({ embeds: [client.lightBlueEmbed(util.cf.winner(winner))] });
		});
	}
});
