...
export default new MessageCommand({
	...
	async execute(client, message) {
		...
		let array: { objId: string; id: string; count: string }[] = [];
		Object.keys(numbers).forEach((key: string) => {
			...
			let e12 = counter.find((nu: { attribs: { class: string } }) => nu.attribs.class === 'rts-nr-int rts-nr-10e12')
				? counter.find((nu: { attribs: { class: string } }) => nu.attribs.class === 'rts-nr-int rts-nr-10e12').children[0].data
				: '';
			let e9 = counter.find((nu: { attribs: { class: string } }) => nu.attribs.class === 'rts-nr-int rts-nr-10e9')
				? counter.find((nu: { attribs: { class: string } }) => nu.attribs.class === 'rts-nr-int rts-nr-10e9').children[0].data
				: '';
			let e6 = counter.find((nu: { attribs: { class: string } }) => nu.attribs.class === 'rts-nr-int rts-nr-10e6')
				? counter.find((nu: { attribs: { class: string } }) => nu.attribs.class === 'rts-nr-int rts-nr-10e6').children[0].data
				: '';
			let e3 = counter.find((nu: { attribs: { class: string } }) => nu.attribs.class === 'rts-nr-int rts-nr-10e3')
				? counter.find((nu: { attribs: { class: string } }) => nu.attribs.class === 'rts-nr-int rts-nr-10e3').children[0].data
				: '';
			let e0 = counter.find((nu: { attribs: { class: string } }) => nu.attribs.class === 'rts-nr-int rts-nr-10e0')
				? counter.find((nu: { attribs: { class: string } }) => nu.attribs.class === 'rts-nr-int rts-nr-10e0').children[0].data
				: '';
			let obj = {
				objId: objId,
				id: id,
		...
		let embed = new MessageEmbed()
			...
			.addField(
				...
				array.find((o) => o.id === 'oil_reserves')!.count +
					` (~${array.find((o) => o.id === 'oil_days')!.count} days = ${Math.floor(parseInt(array.find((o) => o.id === 'oil_days')!.count.replace(',', '')) / 365.25)} years)`
			)
