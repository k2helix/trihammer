...
// 						let condicion = await triviadb.obtener(`${message.guild.id} - ${preguntas}`);
// 						if (condicion == true) array.push(preguntas);
// 					}
// 				});
// 				if (triviadb.tiene(`${message.guild.id} - ${item.number}`)) {
// 					let condicion = await triviadb.obtener(`${message.guild.id} - ${item.number}`);
// 					if (condicion == true)
// 						arr.forEach((number) => {
// 							if (array.includes(number)) ct++;
// 						});
// 					if (ct === 6)
...
// 									} else {
// 										datos.sort((a, b) => {
// 											return b.acertadas - a.acertadas || b.acertadas - a.acertadas;
// 										});
...
// 											usuarios.push(`${parseFloat(x) + 1} - ${user}, acertó ${datos[x].acertadas}`);
// 										}
...
// 														if (!eliminado) console.log('No se encontró nada para borrar.');
...
// 					do item = quiz[Math.floor(Math.random() * quiz.length)];
...
// 							.then((collected) => {
// 								message.channel.send(`¡${collected.first().author} ha acertado!`);
// 								triviadb.establecer(`${message.guild.id} - ${item.number}`, true);
// 								let Schema = new bsonDB.Schema({
// 									id: String,
// 									server: String,
// 									acertadas: Number
...
// 								let Model = new bsonDB.Model('Quiz', Schema);
// 								Model.findOne(
// 									(f) => f.id == collected.first().author.id && f.server == message.guild.id,
// 									(datos) => {
// 										if (!datos) {
