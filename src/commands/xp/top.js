...
module.exports = {
	...
	async execute(client, message, args) {
		if (args[0] === 'global') {
			...
			for (var x in top.slice(0, 10)) {
				let user = message.guild.members.cache.has(top[x].id)
					? message.guild.members.cache.get(top[x].id).user.tag
					: 'User who left the guild (' + top[x].id + ')';
			...
			message.channel.send(
				`** - 🏆 Server Rank -  **\n\`\`\`cs\n${usuarios.join('\n')}\n-------------------------------------\n Your stats:\n Rank ${
					lugar + 1
				}\n Level ${top[lugar].nivel} XP ${top[lugar].xp}  \`\`\``
			);
		}
	}
};
