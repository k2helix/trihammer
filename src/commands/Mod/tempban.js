const { ModelServer, ModelInfrs, ModelTempban } = require('../../utils/models');
module.exports = {
	name: 'tempban',
	...
	async execute(client, message, args) {
		...
		let permiso =
			serverConfig.modrole !== 'none' ? message.member.roles.cache.has(serverConfig.modrole) : message.member.hasPermission('BAN_MEMBERS');
		let adminperms =
			serverConfig.adminrole !== 'none' ? message.member.roles.cache.has(serverConfig.adminrole) : message.member.hasPermission('BAN_MEMBERS');
